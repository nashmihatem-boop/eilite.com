...
export function MarketplaceFAQ({ faqs }: { faqs: FAQItem[] }) {
      ...
      <div className="mx-auto max-w-3xl px-6 text-center lg:px-8">
        ...
        <div className="mt-10 space-y-3 text-left">
          {faqs.map((faq, i) => {
            const isOpen = open === i;
            return (
              <div key={faq.q} className="overflow-hidden rounded-2xl border border-[var(--color-line)] bg-white">
                <button
                  type="button"
                ...
                {isOpen && <div className="px-6 pb-5 text-sm text-[var(--color-ink-soft)]">{faq.a}</div>}
              </div>
            );
          })}
        </div>
